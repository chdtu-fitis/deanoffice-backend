...
ALTER TABLE order_single_approver
    ADD CONSTRAINT fk_order_single_approver_faculty_id FOREIGN KEY (faculty_id) REFERENCES faculty (id);
...
(
    ...
    initiator_approver integer not null,
    faculty_id integer not null,
    active     boolea nnot null
);
...
CREATE UNIQUE INDEX uk_order_approve_template_approvers ON order_approve_template( (approvers->>'main_approver_id, university_approvers') ) ;
...
ALTER TABLE orders ADD CONSTRAINT uk_student_expel_order_id_student_degree_id UNIQUE (order_id, student_degree_id);
...
create table male_surname_cases
(
    id SERIAL primary key,
